//g++ -Wall -std=c++14 -O3 -o fft fft_test.cpp mkl_fft.o \
-lOpenCL -lclFFT -lcudart -lcufft -lfftw3_omp -lfftw3 -fopenmp -lm \
-lmkl_intel_lp64 -lmkl_tbb_thread -lmkl_core -ltbb -lstdc++ -lpthread -ldl \
-I ~/libraries/OpenCL-Headers/ \
...
//////////////////////////////////////////////////////////////////////////////////
    ...
    fftw_complex *in = (fftw_complex*) fftw_malloc(sizeof(fftw_complex)*(X.size()/2));
    for(unsigned int i = 0; i < X.size()/2; ++i) 
    {
    	in[0][i] = X[2*i];
    	in[1][i] = X[2*i+1];
    ...
    p = fftw_plan_dft_1d(X.size()/2, in,
...
//////////////////////////////////////////////////////////////////////////////////
    ...
    try
    {
        ...
        cl_context_properties cprops[] = {
          CL_CONTEXT_PLATFORM,
	  (cl_context_properties)(platformList[PLATFORM_N])(),
	  0};
	...
	OCL_CALL(clfftCreateDefaultPlan(&planHandle, context.get(), dim, clLengths.data()));
	  ...
	  cl_mem x = bufX.get();
        ...
        /* Execute the plan. */
        OCL_CALL(clfftEnqueueTransform(planHandle, CLFFT_FORWARD, 1,
				       &q, 0, NULL, NULL,
				       &x, NULL, NULL));
	/* Release the plan. */
    ...
    {
      std::cerr << "ERROR: " << cpperr.what() << " (" << cpperr.err() << ")" << std::endl;
      return EXIT_FAILURE;
    }
