...
int main(void)
{
  ...
  for(uint i = 0; i < n; ++i) printf("%f %f %f\n", x1[i].x, x1[i].y, x1[i].z);
		  ...
		  float m = 1.0f, float radius = 1.0f)
{
  ...
  for(float t = 0.0f; t <= t_end; t += delta_t)
  {
    ...
    for(uint i = 0; i < n; ++i)
    {
      ...
      while(x1[i].x >= size.x) x1[i].x -= size.x;
      while(x1[i].y >= size.y) x1[i].y -= size.y;
      while(x1[i].z >= size.z) x1[i].z -= size.z;
      while(x1[i].x < 0)    x1[i].x += size.x;
      while(x1[i].y < 0)    x1[i].y += size.y;
      while(x1[i].z < 0)    x1[i].z += size.z;
		  ...
		  float m = 1.0f, float radius = 1.0f)
{
  ...
  float3 *d_x0;
  cudaMalloc(&d_x0, n*sizeof(float3)); CudaCheckError();
  cudaMemcpy(d_x0, x0, n*sizeof(float3), cudaMemcpyHostToDevice); CudaCheckError();
  float3 *d_x1;
  cudaMalloc(&d_x1, n*sizeof(float3)); CudaCheckError();
  cudaMemcpy(d_x1, x1, n*sizeof(float3), cudaMemcpyHostToDevice); CudaCheckError();

  float3 *d_forces;
  cudaMalloc(&d_forces, n*sizeof(float3)); CudaCheckError();

  for(float t = 0.0f; t <= t_end; t += delta_t)
  {
    //printf("t = %f\n", t);
    nbody_forces<<<blocks, threads_per_block>>>(d_x0, d_forces, delta_t, n, m, radius); CudaCheckError();
    nbody_kernel<<<blocks, threads_per_block>>>(d_x0, d_x1, d_forces, n, size); CudaCheckError();
  }

  cudaMemcpy(x1, d_x1, n*sizeof(float3), cudaMemcpyDeviceToHost); CudaCheckError();
}

__global__
void nbody_forces(float3 *x, float3 *forces, float delta_t, uint n, float m, float rad)
{
...
__global__
void nbody_kernel(float3 *x0, float3 *x1, float3 *forces, uint n, float3 size)
{
  uint g_index = blockIdx.x*blockDim.x+threadIdx.x;
  ...
  if(g_index < n)
  {
    ...
    while(s_x1[s_index].x >= size.x) s_x1[s_index].x -= size.x;
    while(s_x1[s_index].y >= size.y) s_x1[s_index].y -= size.y;
    while(s_x1[s_index].z >= size.z) s_x1[s_index].z -= size.z;
    while(s_x1[s_index].x < 0)    s_x1[s_index].x += size.x;
    while(s_x1[s_index].y < 0)    s_x1[s_index].y += size.y;
    while(s_x1[s_index].z < 0)    s_x1[s_index].z += size.z;
