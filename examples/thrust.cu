...
__global__
void mysaxpy(uint n, float a, const float *x, float *y)
{
...
for(uint size = 10000; size < 10000000; size += 10000)
{
  ...
  saxpy_slow(3.0f, d_x, d_y);
    ...
    milliseconds = 0;
    cudaEventElapsedTime(&milliseconds, start, stop); printf("%f", milliseconds);
  //h_y = d_y;
  ...
  nBlocks = (size + blockSize - 1) / blockSize;
    cudaEventRecord(start);
  mysaxpy<<<nBlocks, blockSize>>>
      (size, 3.0f, thrust::raw_pointer_cast(d_x.data()), thrust::raw_pointer_cast(d_y.data()));
    cudaEventRecord(stop);cudaEventSynchronize(stop);
    milliseconds = 0;
    cudaEventElapsedTime(&milliseconds, start, stop); printf("%f\n", milliseconds);
  //h_y = d_y;
  //for(uint i = 0; i < h_y.size(); ++i) std::cout << h_y[i] << std::endl;
}
